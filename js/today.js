...
const TodayPage = (() => {
  ...
  async function render() {
    ...
    if (eqBanner) {
      if (day.exercises && day.exercises.length > 0 && day.dayType !== 'Rest') {
        ...
        day.exercises.forEach((ex, idx) => {
          ...
          if (equip) equipmentMap.set(equip, (equipmentMap.get(equip) || 0) + 1);

          if (ex.isNew) newExercisesList.push(`תרגיל ${exNum}: ${ex.name}`);
          if (ex.isChanged) changedExercisesList.push(`תרגיל ${exNum}: ${ex.name}`);
        });

        if (equipmentMap.size > 0 || newExercisesList.length > 0 || changedExercisesList.length > 0) {
          eqBanner.style.display = 'block';
          const detailsList = eqBanner.querySelector('.equipment-details');
          if (detailsList) {
            let html = '';

            if (newExercisesList.length > 0) {
              html += `<div style="margin-bottom: 6px; color: #10b981; font-weight: bold;">✨ תרגילים חדשים בתוכנית:</div>`;
              newExercisesList.forEach(item => {
                html += `<div style="margin-right: 12px; font-size: 0.85em; color: var(--text-primary); margin-bottom: 2px;">• ${item}</div>`;
              });
            }

            if (changedExercisesList.length > 0) {
              html += `<div style="margin-top: 6px; margin-bottom: 6px; color: #f59e0b; font-weight: bold;">🔄 תרגילים ששונו/עודכנו:</div>`;
              changedExercisesList.forEach(item => {
                html += `<div style="margin-right: 12px; font-size: 0.85em; color: var(--text-primary); margin-bottom: 2px;">• ${item}</div>`;
              });
            }

            if (equipmentMap.size > 0) {
              if (newExercisesList.length > 0 || changedExercisesList.length > 0) {
                html += `<div style="margin-top: 8px; margin-bottom: 6px; color: var(--text-secondary); font-weight: bold;">🏋️ ציוד נדרש להיום:</div>`;
              }
              equipmentMap.forEach((count, equip) => {
                html += `<div style="margin-right: 12px; font-size: 0.85em; color: var(--text-primary); margin-bottom: 2px;">• ${equip}</div>`;
              });
            }
    ...
    // Build exercises list or rest day
    const container = document.getElementById('exercises-container');
    const restCard = document.getElementById('rest-day-card');

    if (day.dayType === 'Rest' || day.exercises.length === 0) {
      container.style.display = 'none';
      restCard.style.display = 'block';

      // Update rest day checkbox
      const restCheckbox = document.getElementById('rest-complete-checkbox');
      if (restCheckbox) {
        restCheckbox.checked = !!currentTracking.completed;
      }
      
      const restTitle = document.getElementById('rest-day-title');
      const restSub = document.getElementById('rest-day-sub');
      
      if (day.dayType === 'Active Recovery') {
        if (restTitle) restTitle.textContent = '🧘 יום התאוששות פעילה';
        if (restSub) restSub.textContent = 'יום קל מיועד להמרצת דם, מתיחות והתאוששות. בצע הליכה קלה או מתיחות במידת הצורך.';
      } else {
        if (restTitle) restTitle.textContent = '😴 יום מנוחה';
        if (restSub) restSub.textContent = 'תן לשרירים להשתקם. תזונה ושינה הן מפתח להתקדמות!';
      }
      ...
      container.innerHTML = day.exercises.map((ex, idx) =>
        renderExerciseCard(ex, idx, currentTracking)
      ).join('');

      // Bind exercise notes and input listeners
      bindExerciseInputs();
    }
