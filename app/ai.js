...
const AIModule = {
  ...
  // Cache: { key -> { text, expiresAt } }
  _cache: {},
  // If rate-limited, don't retry until this timestamp
  _rateLimitedUntil: 0,
  ...
  _setCached(key, text, ttlMs = 10 * 60 * 1000) { // 10 min default TTL
    this._cache[key] = { text, expiresAt: Date.now() + ttlMs };
  },
  ...
  async callAPI(prompt, systemPrompt = '') {
    ...
    // Respect rate-limit window — don't hammer the API
    ...
    try {
      ...
      // Handle 429 — respect retry-after, mark module temporarily unavailable
  ...
  async getMotivation(context = 'general') {
    ...
    if (!this.available || !this.session || this._isRateLimited()) return fallback;

    // Return cached response if still fresh (10 min TTL)
    const cacheKey = `motivation_${context}`;
    ...
    try {
      ...
      const text = result?.trim() || fallback;
      this._setCached(cacheKey, text); // Cache for 10 min
      return text;
    } catch(e) {
      return fallback;
    }
